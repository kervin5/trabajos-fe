...
import { getJobsFeed, getJobUrl } from 'src/lib/jobs';
import { NextApiRequest, NextApiResponse } from 'next';

const SITE_ROOT = process.env.NEXT_PUBLIC_URL as string;
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const jobs = await getJobsFeed(); // call the backend and fetch all jobs
